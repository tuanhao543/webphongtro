...
def timkiem(request, ma_kh):
    if request.method == 'POST':
        search_title = request.POST.get('search_title', '')
        # __icontains = tìm tư có trong tieu đề và không phân biệt in hay thương 
...
def danhmucquan(request, madanhmucquan, ma_kh):
    ...
    ma_kh = request.session.get('khachhang_id')
    khachhang = Khachhang.objects.get(ma_kh=ma_kh)
    selected_danhmucquan = Danhmucquan.objects.get(madanhmucquan=madanhmucquan) 
    phongq = Phong.objects.filter(danhmucquan=selected_danhmucquan)
    context = {
        'danhmucquans': danhmucquans,
        'phongq': phongq,
        'selected_danhmucquan': selected_danhmucquan,  # Pass the selected object
        'khachhang': khachhang
    }
...
def danhmucgia(request, madanhmucgia, ma_kh):
    ...
    ma_kh = request.session.get('khachhang_id')
    khachhang = Khachhang.objects.get(ma_kh=ma_kh)
    selected_danhmucgia = Danhmucgia.objects.get(madanhmucgia=madanhmucgia)
    phongg = Phong.objects.filter(danhmucgia=selected_danhmucgia) 
    
    context = {
        'danhmucgias': danhmucgias,
        'phongg': phongg,
        'selected_danhmucgia': selected_danhmucgia,  # Pass the selected object
        'khachhang': khachhang
    }
...
class AiAssistantView(View): 
    ...
    def process_input(self, NguoiDung, request):
        if 'chào' in NguoiDung:
            ...
            return {'response': f'Đây là kết quả tìm kiếm "{search_keyword}" của bạn', 'redirect_url': redirect_url}
        else:
